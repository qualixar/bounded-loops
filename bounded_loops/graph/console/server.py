...
if TYPE_CHECKING:
    ...
    # (graph_runtime_facade.py imports cli_graph._load_plan_from_run_dir; cli_graph.py
    # lazily imports this package's cmd_graph_console). Mirrors cli_graph_approve.py's
    # own deferred-import discipline.
...
def open_console_run(run_dir: Path) -> tuple[GraphRunIdentity, "LocalGraphRuntimeFacade"]:
    ...
    validation) and ``cli_graph._load_plan_from_run_dir`` (identity reconstruction)
    UNCHANGED — the exact two calls ``cmd_graph_approve`` already trusts. Neither
    check is re-implemented here; a failure in either raises ``ConsoleOpenError``
    with the original message, so the CLI layer has one exception type to catch.
    """
    from bounded_loops.graph.application.graph_runtime_facade import LocalGraphRuntimeFacade
    ...
    from bounded_loops.graph.cli_graph import _load_plan_from_run_dir  # deferred: avoid import cycle
    ...
    try:
        _plan, identity, _meta = _load_plan_from_run_dir(run_dir.resolve())
...
class ConsoleServer(ThreadingHTTPServer):
    ...
    CRIT finding (documented, deliberately NOT "fixed" with a rate limiter): this
    console's own threat model treats other LOCAL processes as untrusted (the
    token is what gates them) — but ``ThreadingHTTPServer`` spawns one thread per
    ACCEPTED connection with no cap, so another unprivileged local process could
    still open many connections and exhaust threads/file descriptors before ever
    reaching the token check, denying the real operator service for as long as
    the console needs to stay up. A bounded worker pool or a per-source rate
    limiter would close this, but both are disproportionate scope for a console
    this LLD calls "minimal" three times over, used for a handful of clicks
    before it exits on its own. ``request_queue_size`` is set explicitly (rather
    than left at the stdlib default) so the OS-level backlog is an intentional,
    reviewable number, not an implicit one.
    """
