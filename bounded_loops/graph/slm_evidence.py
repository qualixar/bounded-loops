...
def evidence_for_run(workspace: Workspace, run_id: str) -> dict[str, Any]:
    ...
    projection, demonstration = _project(run_dir)
    return evidence_document(
        projection,
        workspace_id=workspace_digest(workspace.root),
        terminal_at=_terminal_at(run_dir),
    ...
    for entry in sorted(runs_root.iterdir(), reverse=True):
        ...
        try:
            ...
            found.append({
                ...
                "terminal_at": _terminal_at(entry),
            })
...
def _safe_run_dir(workspace: Workspace, run_id: str) -> Path:
    try:
        ...
        raise EvidenceUnavailable(f"not a usable run id: {exc}") from exc
    if run_dir.is_symlink() or not run_dir.is_dir():
        ...
        raise EvidenceUnavailable(f"no such run in this workspace: {run_id!r}")
    return run_dir


def _project(run_dir: Path) -> tuple[Any, bool]:
    ...
    try:
        ...
        raise EvidenceUnavailable(f"cannot reconstruct the plan for this run: {exc}") from exc
    ...
    try:
        ...
        raise EvidenceUnavailable(f"cannot read this run's receipts: {exc}") from exc

    return projection, bool(run_meta.get("demonstration"))


def _terminal_at(run_dir: Path) -> str:
    """The timestamp of the LAST receipt — when the run actually stopped.

    Taken from the log rather than from the file's mtime: mtime changes if the directory is
    copied, archived or restored, and a consumer keyed on it would see the same run finish
    twice. The receipt says when it happened and keeps saying so.
    """
    last: str | None = None
    try:
        with (run_dir / "controller-events.jsonl").open(encoding="utf-8") as handle:
            for line in handle:
                marker = '"timestamp":'
                start = line.find(marker)
                if start == -1:
                    continue
                quote = line.find('"', start + len(marker))
                end = line.find('"', quote + 1)
                if quote != -1 and end != -1:
                    last = line[quote + 1:end]
    except OSError as exc:
        raise EvidenceUnavailable(f"cannot read this run's event log: {exc}") from exc

    if last is None:
        raise EvidenceUnavailable("this run's event log carries no timestamped receipt")
    return last
