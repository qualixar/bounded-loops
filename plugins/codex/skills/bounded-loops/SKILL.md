...
- `loop.yaml` defines runner, gate, rung, and forbid rules.
- `bounds.yaml` enforces iteration, no-progress, wall-clock, token, sandbox,
  quarantine, schema, tracing, and approval bounds.
- `PROMPT.md` is the per-lap spec.
- `seed/` is copied into a scratch workspace.
- `.ledger.jsonl` records terminal and continue decisions.
- `composite` gates support `mode: all` in v1.

## Reporting rules

- `DONE` means success.
- `HALT` means a bound tripped.
- `PAUSE` means approval is required.
- `KILLED` means the kill switch tripped.
- `ERROR` means the runner or gate could not execute.
