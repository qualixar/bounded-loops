...
def test_run_once_agent_claimed_done_always_false_even_on_turn_completed(tmp_path):
    ...
    with patch("bounded_loops.adapters.runners.codex.ProcessTurn.start", return_value=_fake_turn(stdout=jsonl)):
        runner = CodexRunner()
...
def test_run_once_writes_agent_output(tmp_path):
    with patch("bounded_loops.adapters.runners.codex.ProcessTurn.start", return_value=_fake_turn(stdout="raw stream")):
        runner = CodexRunner()
        runner.run_once(_spec(), _ctx(tmp_path))
...
def test_run_once_timeout_raises_runner_error(tmp_path):
    with patch("bounded_loops.adapters.runners.codex.ProcessTurn.start", return_value=_fake_turn(state=TurnState.TIMED_OUT)):
        runner = CodexRunner()
...
def test_run_once_missing_binary_raises_runner_error(tmp_path):
    with patch("bounded_loops.adapters.runners.codex.ProcessTurn.start", side_effect=OSError("no such file")):
        runner = CodexRunner()
...
def test_run_once_builds_argv_with_sandbox_mode(tmp_path):
    with patch("bounded_loops.adapters.runners.codex.ProcessTurn.start", return_value=_fake_turn(stdout="")) as mock_start:
    ...
    assert mock_start.call_args.kwargs["input_text"] == "# Goal\nFix the bug\n\n# Steps\n1. step A"
...
def test_run_once_turn_failed_raises_runner_error(tmp_path):
    ...
    with patch("bounded_loops.adapters.runners.codex.ProcessTurn.start", return_value=_fake_turn(returncode=1, stdout=jsonl)):
        runner = CodexRunner()
...
def test_run_once_nonzero_exit_without_failure_event_raises_runner_error(tmp_path):
    with patch(
        "bounded_loops.adapters.runners.codex.ProcessTurn.start",
        return_value=_fake_turn(returncode=2, stdout="", stderr="bad option"),
    ):
        runner = CodexRunner()
