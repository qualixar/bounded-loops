"""Acceptance tests for loops/autogen-example/ (pattern shared with the other framework-example loops).

Package/import name note: "AutoGen" converged into Microsoft Agent Framework
(agent-framework on PyPI, agent_framework import root — verified directly
against PyPI/GitHub at implementation time, 2026-07-05). The skip guard
below imports "agent_framework", matching this loop's real glue.py, not the
literal "autogen" name the loop folder is still called after (folder name
kept for continuity; the actual framework wired is Agent Framework)."""
import subprocess
import sys
import pytest
from pathlib import Path

LOOP_DIR = Path(__file__).parent.parent.parent / "loops" / "autogen-example"


def test_bl_lint_passes_without_framework_installed():
    """Manifest validation never imports the framework — must pass in CI
    even when agent_framework isn't installed."""
...
def test_run_only_with_framework_installed(tmp_path):
    """The real end-to-end proof — SKIPPED, not failed, if agent_framework
    isn't installed. This is the test that actually proves the copy-paste
    payload works, for a CI runner that opts in with the framework
    installed."""
