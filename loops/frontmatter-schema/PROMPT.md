# Frontmatter Schema: fix output.json to match schema.json

Goal: make `output.json` validate against `schema.json`.

Steps each turn:
  1. Read `schema.json` to see the required contract: `title` (string),
     `date` (string, format date), `slug` (string, pattern
     `^[a-z0-9-]+$`), `tags` (array of strings, minItems 1). All four are
...
Do not loosen `schema.json` (e.g. removing `required` entries, the `slug`
pattern, or `additionalProperties: false`) to make the broken data pass —
