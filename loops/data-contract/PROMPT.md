# Data Contract: fix output.json to match schema.json

Goal: make `output.json` validate against `schema.json`.

Steps each turn:
  1. Read `schema.json` to see the required contract: `id` (integer),
     `email` (string, format email), `created_at` (string, format
     date-time), `role` (string, enum admin/member/viewer). All four are
     required; no extra properties are allowed.
...
Do not loosen `schema.json` (e.g. removing `required` entries, the `role`
enum, or `additionalProperties: false`) to make the broken data pass —
the contract is the ground truth, not the data.
